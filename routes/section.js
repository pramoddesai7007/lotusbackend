...
// Get default section API
router.get('/section/default', async (req, res) => {
    try {
        // Find the default section
        const defaultSection = await Section.findOne({ isDefault: true });

        if (!defaultSection) {
            return res.status(404).json({ message: 'Default section not found' });
        }

        res.json(defaultSection);
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
});
